"""Copy of shiny app with few changes"""
...
EMPTY_TX = Transactions.empty_transactions_factory()
...
def filter_funcs_factory(
    ...
    def filtered_transactions():
        ...
        if in_date_range_transactions.size() == 0:
            logging.warning(f"No transactions found for '{time_unit}' time unit in given date range {start_date} to {end_date}.")
            return None

        filtered_in_transactions = in_date_range_transactions.containing_tags(filter_in_tags)
        if filtered_in_transactions.size() == 0:
            logging.warning(f"No transactions found for '{time_unit}' time unit containing {filter_in_tags} tags.")
            return None

        filtered_in_and_out_transactions = filtered_in_transactions.not_containing_tags(filter_out_tags,
                                                                                        empty_tags_strategy='all_true')
        if filtered_in_and_out_transactions.size() == 0:
            logging.warning(f"No transactions found for '{time_unit}' time unit after filtering out {filter_out_tags} tags.")
            return None
