...
class HSBCTransformerTest(unittest.TestCase):
    def test_transform_hsbc_transactions(self):
        ...
        expected_output = pd.DataFrame({
            'id': [11, 12, 13],
            'datetime': pd.to_datetime(['2022-01-01 00:00:00', '2022-01-02 00:00:00', '2022-01-03 00:00:00']),
            'amount': [100.0, 200.0, 300.0],
        ...
        df_monzo = pd.DataFrame(
            {
                    'id': [1, 2, 3],
                    'date': [date(2022, 1, 1), date(2022, 1, 2), date(2022, 1, 3)],
                    'time': [time(10, 0, 0), time(11, 0, 0), time(12, 0, 0)],
...
class RevoTransformerTest(unittest.TestCase):
    ...
    def test_transform_revo_transactions(self):
        df_revo = pd.DataFrame(                {
                    ...
                    'start_date': pd.to_datetime(['2022-01-01 10:00:00', '2022-01-02 11:00:00', '2022-01-03 12:00:00']),
                    'completed_date': pd.to_datetime(['2022-01-01 10:00:00', '2022-01-02 11:00:00', '2022-01-03 12:00:00']),
                    'description': ['Description 1', 'Description 2', 'Description 3'],
        ...
        expected_output = pd.DataFrame({
            ...
            'description': ['type: Type 1, product: Product A, completed_date: 2022-01-01 10:00:00, description: Description 1, fee: 10.0, state: State 1, balance: 1000.0',
